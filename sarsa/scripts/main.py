...
Q = np.random.rand(a, b) * theta_0 # Qは表.  初期値としてランダムな値(0.0以上、1.0未満)を設定する.
...
def simple_convert_into_pi_from_theta(theta):
    ...
    for i in range(0, m):
        ...
        # nansumは欠損値nanを除外したsumのこと(https://note.nkmk.me/python-numpy-nansum/)
...
def get_action_and_s_next(s, Q, epsilon, pi_0):
    ...
    # 行動を決める
    if np.random.rand() < epsilon: # randは0.0以上、1.0未満   εも同様
        # εの確率でランダムに動く
        next_direction = np.random.choice(direction, p=pi_0[s, :])
    else:
        # Qの最大値の行動を採用する
        next_direction = direction[np.nanargmax(Q[s, :])] # nanargmaxは最大値を持つ要素のインデックスを返す.
...
while is_continue:  # is_continueがFalseになるまで繰り返す
    ...
    reach_step.append([len(s_a_history) - 1])
    # 100エピソード繰り返す
    episode = episode + 1
    if episode > 100:
        break

print("plot reatch step transition ----->")
plt.plot(reach_step)
plt.show(2)
